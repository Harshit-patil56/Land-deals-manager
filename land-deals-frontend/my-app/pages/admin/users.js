...
export default function AdminUsers() {
  ...
  const handleReset = async (userToReset) => {
    if (!hasPermission(user, PERMISSIONS.USERS_EDIT)) {
      toast.error('You do not have permission to reset passwords')
      return
    }
    const pw = prompt(`Enter new password for user "${userToReset.username}":`)
    if (!pw) return
    if (pw.length < 6) {
    ...
    try {
      await adminAPI.updateUser(userToReset.id, { password: pw })
